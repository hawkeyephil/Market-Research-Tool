...
exchange_Rate_Data = panda.DataFrame(columns=['Currency', 'Currency Code', 'Date', 'Exchange Rate to USD'])

#Currency codes for X Currency to U.S. Dollars spot exchange rate from the FRED database 
currency_Codes_ToUSD = [('Japanese Yen', 'DEXJPUS'), ('Canadian Dollar', 'DEXCAUS'), ('Swiss Franc', 'DEXSZUS')] 
#Currency codes for U.S. Dollars to X Currecny spot exchange rate from the FRED database 
currency_Codes_FromUSD = [('Euro', 'DEXUSEU'), ('Pound Sterling', 'DEXUSUK'), ('Australian Dollar', 'DEXUSAL')] 
