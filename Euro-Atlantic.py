...
euro_Atlantic_Codes = [('Canada', 'Canadian Dollar', 'DEXCAUS', 'fred', 'na', 'NGDPRSAXDCCAQ', 'fred', 0, 'CANCPALTT01CTGYM', 'fred'), ('Switzerland', 'Swiss Franc', 'DEXSZUS', 'fred', 'na', 'CPMNACSAB1GQCH', 'fred', 1000000, 'CHECPALTT01CTGYM', 'fred')]
...
def macro_Variable_Collector(macro_Variable_Codes): 
    ...
    for macro_Variable_Code, macro_Variable_Source, special_Behavior in macro_Variable_Codes: 
        ...
        #macro_Variable_Recent = str(macro_Variable_Recent)  
...
def exchange_Rate_Generator(exchange_Rate_Codes):  
    ...
    for index, row in exchange_Rates.iterrows(): 
        ...
        exchange_Rates_Adjusted.append(str(row["macro_Variable_Recent"]) + ' (' + row["macro_Variable_Date"] + ')')
...
def output_Generator(output_Codes): 
    ...
    for index, row in outputs.iterrows():        
        if(row["special_Behavior"] != 0): 
            row["macro_Variable_Recent"] = int(row["macro_Variable_Recent"])/row["special_Behavior"] 
        outputs_Adjusted.append(str(row["macro_Variable_Recent"]) + ' (' + row["macro_Variable_Date"] + ')')
...
def inflation_Rate_Generator(inflation_Rate_Codes): 
    ...
    for index, row in inflation_Rates.iterrows(): 
        inflation_Rates_Adjusted.append(str(row["macro_Variable_Recent"]) + ' (' + row["macro_Variable_Date"] + ')')
...
def report_Generator(region_Codes): 
    ...
    output_Codes = [] 
    inflation_Codes = [] 
    for economy, currency, exchange_Rate_Code, exchange_Rate_Source, exchange_Rate_Inversion, output_Code, output_Source, adjustment, inflation_Code, inflation_Source in region_Codes: 
        exchange_Rate_Codes.append((exchange_Rate_Code, exchange_Rate_Source, exchange_Rate_Inversion)) 
        output_Codes.append((output_Code, output_Source, adjustment)) 
